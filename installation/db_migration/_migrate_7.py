...
async def main():
    ...
    db_stream = await db_session.execute(sa.select(ArtefactMetadata).where(
        ArtefactMetadata.type == dso.model.Datatype.RESCORING,
    ))
    ...
    for idx, partition in enumerate(db_stream.partitions(size=50)):
        ...
        for row in partition:
            ...
            artefact_metadatum_raw.data_key = hashlib.sha1(
                artefact_metadatum.data.key.encode(),
                usedforsecurity=False,
            ).hexdigest()
            artefact_metadatum_raw.id = artefact_metadatum.id
